...
app.put("/jokes/:id",(req,res)=>{
  ...
  for (let i = 0; i < jokes.length; i++) {
    if (jokes[i].id === id) {
      jokes[i] = updatedJoke;
      break; // Once the joke is updated, you can exit the loop
    }
  }
...
app.delete("/jokes/:id",(req,res)=>{
  ...
  const deleted =jokes.splice(id-1,1)
...
app.delete("/all",(req,res)=>{
  if(masterKey===req.query.masterKey){
    jokes=[]
    res.sendStatus(200)
  }
  else{
    console.log(req.query.masterKey)
    res.status(404).json("Incorrect masterkey")
  }
})
app.listen(port, () => {
